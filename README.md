...
- `slide/index.html`: RDS 車輪速度制御モデル（`wheel.m`）の解説スライドWebアプリケーションです．SVG 制御ブロックダイアグラム，MathJax 3 による LaTeX 数式表示，モータ状態空間モデル $\dot{x} = Ax + Bu$，$y = Cx + d$，PID/FF制御，60Hz LPF 速度推定の解説と，ブラウザ上で動くリアルタイム車輪速度制御シミュレータが含まれています．
