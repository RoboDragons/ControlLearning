%% wheel.m (総合課題用・穴あきスクリプト)
%  RDS E2023 車輪速度制御モデル 演習課題（統合シミュレーション）
%
%  【演習の進め方】
%  各ステップを個別に学習・確認したい場合は，以下の個別課題スクリプトを順に実行してください：
...
%  
%  本スクリプト (wheel.m) は上記すべてを統合したフルシミュレーションです．
%
%  【制御構造】
%    v_ref [rad/s]
%        ↓
...
% TODO: 【課題 1-1】同定パラメータから連続時間モデルの a_m, b_m を求める式を完成させなさい．
% (ヒント: e^(a_m * Ts) = 0.9504, ゲイン K_c = 0.01039 / (1 - 0.9504) = -b_m / a_m)
a_m = ???;
b_m = ???;

% TODO: 【課題 1-2】連続時間状態空間行列 Ac, Bc および出力行列 Cc, Dc を定義しなさい．
% Ac = [0, K_enc; 0, a_m], Bc = [0; b_m]
Ac = [0, ???;
      0, ???];
Bc = [0;
      ???];
...
for k = 1:N
    ...
    % TODO: 【課題 2-3】全制御入力 a_pwm_raw を求め，±PWM_MAX で飽和処理した a_pwm を作成しなさい．
    a_pwm_raw = ???;
    a_pwm     = max(-PWM_MAX, min(PWM_MAX, ???));
    
    % (3) プラント更新: x[k+1] = Ad * x[k] + Bd * a_pwm
    x = Ad * x + Bd * a_pwm;
    
    % TODO: 【課題 3】出力方程式 y = Cx + d に基づき，エンコーダ位置 x_enc を計算しなさい．
    x_enc = ???;
    ...
    % TODO: 【課題 4-1】後退差分 diff_enc を求め，1秒あたりのパルス数 backward_diff [pulse/s] に換算しなさい．
    diff_enc   = ???;
    x_enc_prev = x_enc_meas;
    backward_diff = diff_enc * ???;  % [pulse/s]
    
    % TODO: 【課題 4-2】2次 LPF の差分方程式 lpf_y_new を計算しなさい．
    % (差分方程式: y[k] = b0*u[k] + b1*u[k-1] + b2*u[k-2] - a1*y[k-1] - a2*y[k-2])
    lpf_u = [backward_diff, lpf_u(1), lpf_u(2)];
    lpf_y_new = ???;
    lpf_y = [lpf_y_new, lpf_y(1), lpf_y(2)];
    
    % TODO: 【課題 4-3】LPF 出力 [pulse/s] をモータ角速度 v_fb [rad/s] に単位変換しなさい．
    v_fb = ??? * lpf_y_new;
...
figWave = figure('Name', 'RDS E2023 車輪速度制御モデル 演習課題', 'Position', [150, 100, 900, 700]);
...
ylabel('角速度 [rad/s]'); title('速度追従性能 (v\_ref vs v\_fb)'); legend('Location', 'best'); grid on; xlim([0, t_end]);

ax2 = subplot(3, 1, 2);
plot(t, log_a_pwm, 'm-', 'LineWidth', 1.2);
yline( PWM_MAX, 'k--', 'PWM MAX (+3198)'); yline(-PWM_MAX, 'k--', 'PWM MIN (-3198)');
ylabel('制御入力 a\_pwm'); title('制御入力 a\_pwm [count]'); grid on; xlim([0, t_end]);

ax3 = subplot(3, 1, 3);
plot(t, log_x_enc, 'c-', 'LineWidth', 1.2);
ylabel('エンコーダ [pulse]'); xlabel('時間 [s]'); title('エンコーダ累積パルス y = Cx + d (x\_enc)'); grid on; xlim([0, t_end]);
